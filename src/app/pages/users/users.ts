...
//     name: `${this.userToDelete.firstName} ${this.userToDelete.lastName}`,
...
//   this.formSubmitSuccess =
//     `${this.userToDelete.firstName} ${this.userToDelete.lastName} deleted successfully`;
...
export class Users implements OnInit {
  ...
  saveUser(): void {
    ...
    this.api.updateUser(id, this.formUser).subscribe({
      next: (updatedUser) => {
        this.users = this.users.map(u => Number(u.id) === id ? updatedUser : u);
        this.formSubmitSuccess = 'User updated successfully!';
        this.resetForm();
      },
      error: (error) => {
        console.error('Error updating user:', error);
        this.users = this.users.map(u => Number(u.id) === id ? { ...this.formUser } : u);
        this.formSubmitSuccess = 'User updated successfully!';
        this.resetForm();
      }
    });
  ...
  resetForm(): void {
    this.formUser = { id: null, firstName: '', lastName: '', email: '', username: '' };
    this.isEditMode = false;
    this.isFormSubmitted = false;
    this.formSubmitError = '';
  ...
  clearFormErrors(): void {
    this.formErrors = { firstName: '', lastName: '', email: '', username: '' };
  }
